...
namespace ScrSh
{
    internal sealed class Program
    {
        ...
        private static void Main(string[] _args)
        {
            ...
            {
                //format depends on whether the path is a directory, it's png by default
            ...
            if (System.IO.Directory.Exists(path))
            {
                String date = String.Format("{0:yyyy-MM-dd_HHmmss}", DateTime.Now);
                String filename = String.Concat(date, ".", format);
            ...
            try
            {
                ...
                if (output != null)
                {
                    //we need some argument manipulation here due to nature of Process.Start method
                    List<String> output_args = new List<String>(output.Replace("\"", "").Split(" ".ToCharArray()));
                    output_args.Add(String.Format("\"{0}\"", path));
                    String program = output_args.First();
                    String program_args = output_args.Skip(1).Aggregate((i, j) => i + " " + j);
                    try
                    {
                        System.Diagnostics.Process.Start(program, program_args);
                    }
                    catch (System.ComponentModel.Win32Exception e)
                    {
                        Console.Error.WriteLine(String.Format("Could not run program \"{0}\" ({1})", output, e.Message));
                        Environment.Exit(EXIT_FAILURE);
                        return;
                    }
                }

...
//
// Version 2 (October 2014), revised by mard for Zandronum 1.2
// Compatible with Zandronum launchers and servers.
//
//------------------------------------------------------------------------------------------------------------+
//  REFERENCE:
...
class Huffman
{
  ...
  public static function decode($data_string)
  {
    ...
    //echo "Padding length: $padding_length <BR> strlen(binary_string): ". strlen($binary_string);
    ...
    while ($binary_string)
    {
      foreach (self::$huffman_table as $ascii => $binary_path)
      {
        ...
        if (substr($binary_string, 0, $binary_length) === $binary_path)
        {
          ...
          //$decoded_string .= $ascii . ", ";
          //$decoded_string .= dechex($ascii) . " ";
          //$decoded_string .= pack('C', $ascii);
